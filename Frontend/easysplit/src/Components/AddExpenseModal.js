...
function AddExpenseModal(props) {
    ...
    */
    function computeSplit() {
        ...
        if (splitMethod === 'exact') {
        ...
        else if (splitMethod === 'percent') {              // Do the same as above but when split method is percent
            ...
            for (let li of lis) {
                ...
                const textnode = document.createTextNode(`$` + `${(((li.children[1].value) / 100) * totalAmount).toFixed(2)}`); // Make a DOM element that will display split amounts 
                node.appendChild(textnode);
                if (!li.children[2]) {                                                  // If the split amount has not been computed yet, it will append the split amount to the list
                    li.appendChild(node);
                }
                else {                                                                   // If it has been computed before, it will change the split amount to the latest computed one
        ...
        if (splitMethod === 'equal') {
            ...
            setAmountAfterSplit(() => {
                if (splitWithArr.length < 2) {                  // If there is 0 or 1 emails, the split amount remains the same as the total bill amount
                    ...
                    setFinalSplitAmount((amount.current.value / splitWithArr.length).toFixed(2));   // If emails >=2 split the total amount equally between all emails
